...
public class RuneAssistSuggestionSource
{
    ...
    public void getSuggestionAsync(Consumer<Suggestion> consumer)
    {
        ...
        new Thread(() ->
        {
            ...
            if (ExperimentService.isAllowed(displayName) && experimentService.hasActive(displayName))
            {
                ...
                {
                    final Suggestion finalExpSuggestion = expSuggestion;
                    clientThread.invokeLater(() -> consumer.accept(finalExpSuggestion));
                    return;
                }
            }
    ...
    /** Is there already a live offer for this item on this side (any slot)? Used by the
     * experiment ladder to avoid re-suggesting a rung whose offer is already out working. */
    private static boolean hasOpenOfferFor(long[][] offersBySlot, int itemId, boolean buy)
    {
        ...
        {
            if (o == null) continue;
            if ((int) o[0] == itemId && (o[1] != 0) == buy) return true;
        }
        return false;
    }
