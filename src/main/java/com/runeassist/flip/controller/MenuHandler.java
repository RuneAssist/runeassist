...
public class MenuHandler {
    ...
    private final PortfolioMenuEntries portfolioMenuEntries;

    public void injectPriceGraphMenuEntry(MenuEntryAdded event) {
        if (!playerLocationController.isNearGE() || !config.priceGraphMenuOptionEnabled()) {
            return;
        }
    ...
    public void injectInventoryPortfolioMenuEntry(MenuEntryAdded event) {
        ...
        portfolioMenuEntries.injectExamineMenus(event);
    }
    ...
    private boolean isGeTradableItem(int itemId) {
        ...
        if (item.getNote() != -1) {
            ...
            if (unnotedItemId > 0) {
                return client.getItemDefinition(unnotedItemId).isGeTradeable();
            }
        }
        return false;
    }
    ...
    public void injectConfirmMenuEntry(MenuEntryAdded event) {
        if (!config.disableLeftClickConfirm() || !grandExchange.isOpen() || offerDetailsCorrect()) {
            return;
        }
    ...
    public void injectSlotActionSwapMenuEntry(MenuEntryAdded event) {
        if (!config.slotActionSwap() || !grandExchange.isOpen() || grandExchange.isSlotOpen()) {
            return;
        }
        Suggestion suggestion = suggestionManager.getSuggestion();
        if (suggestion == null || (!suggestion.isAbortSuggestion() && !suggestion.isModifySuggestion())) {
            return;
        }
    ...
    private boolean offerDetailsCorrect() {
        ...
        if (editorItem == offerManager.getViewedSlotItemId() && offerManager.getViewedSlotItemPrice() > 0) {
