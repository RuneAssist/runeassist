...
- **(linked accounts, active decant plan)** Suggestions may include buy → decant at Bob Barter → sell for supported potions consolidated into four doses. Only the active potion family's inventory counts are sent to `/v1/suggestion`. A locally observed, dose-conserving conversion at Bob is stored under your linked account through `/v1/account/decant-conversion`, including input/output IDs, quantities and observation time. This transfers purchase cost without fabricating GE trades. The plan and pending conversion remain in local RuneLite configuration until reconciled; retries reuse the same event ID. This personal history is separate from optional training contribution. No general inventory or bank contents are uploaded for decanting.
